...
# TEMPORARY Gemini API key (for local testing only)
# ⚠️ Do NOT commit this key to GitHub or share publicly.
gemini_api_key = "AIzaSyCkarJ3Y8pzrK_rYMtId6xMkVUu49pLmLA"

if not gemini_api_key or "AIza" not in gemini_api_key:
    print("ERROR: GEMINI_API_KEY not found or invalid in environment variables or code")
else:
            ...
            for chunk in response_generator:
                ...
                if hasattr(chunk, "text"):
                    if chunk.text:  # Only yield non-empty text
                        yield chunk.text #yield(explain in easy terms)=>yield is a generator function that returns a generator object that can be iterated over. It is used to create a sequence of values that can be iterated over, such as a list, tuple, or range object.
...
if __name__ == "__main__":
    ...
    query = "Skills required:Strong knowledge and elevant programming experience with Python, PyTorch, and Scikit-Learn and other ML and DL libraries, mainly for Multimodal AI tasks.Expertise in Neural Machine Translation task using LLMs, especially in Low-resource languages. The candidate is expected to have experience in finetuning and applying modifications to SOTA open-source LLMs.Demonstratable skills of successfully applying SOTA DL, i.e. LLM-based, models to multimodal problems, employing combination of audio, textual and visual data, with example in the form of published papers and journals. Detailed understanding of network architectures, deployment modes, data augmentation and preparation, and performance analysis of model architectures. Strong analytical and problem-solving skills with knowledge of algorithms, signal processing, mathematics for machine learning, probability, statistics and linear algebra. Excellent interpersonal skills with effective communication and presentation skills. Good to have Skills: Demonstrate research capabilities through relevant publication record in leading journals and conferences. Impact driven mindset and ability to work and learn in a collaborative and diverse environment. Programming knowledge of using data structure and OOP concepts proficiently in Python, C and C++. Participation in Kaggle and other open-source grand challenge competitions."
